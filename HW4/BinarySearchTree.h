...
public:
	...
	virtual void postOrderNumbers(Node *u);
	virtual Node* getNode(T x);
	DLList<T> getLE(T x);
	void getLE2(Node *u, T x,DLList<T> * list);
	bool check;
};
...
T BinarySearchTree<Node,T>::find(T x) {
 ...
 template<class Node, class T>
   void BinarySearchTree<Node,T>::getLE2(Node *u, T x, DLList<T> * list){
   if(u == nil) return;
   if(!check)return;
   getLE2(u->left,x,list);
   if(compare(u->x,x) == 0){
     list->add(u->x);
     check = false;
   }
   else if(compare(u->x,x) == -1){
    list->add(u->x);
   }
   else check = false;
   getLE2(u->right,x,list);
 }
